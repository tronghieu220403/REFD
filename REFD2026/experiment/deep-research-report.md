...
Các đúng/sai của điển hình đường dẫn “Known Folders” và biến môi trường được mô tả trong tài liệu của entity["company","Microsoft","technology company"].   
...
Định nghĩa (Goh–Barabási): \(B=\frac{\sigma-\mu}{\sigma+\mu+\varepsilon}\) với \(\mu=\text{mean}(\tau),\sigma=\text{std}(\tau)\). \(f_burstiness_B=B\).
...
Định nghĩa: \(f_temp_event_count = |\{i: p_i \in temp/cache\}|\), temp/cache ≈ `\Users\<*>\AppData\Local\Temp\` (và các vùng cache/tạm tương đương theo taxonomy).
