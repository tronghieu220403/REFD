...
namespace file
{
    ...
    public:
        ZwFile(const std::WString& current_path);
        ZwFile() = default;
        ...
        std::WString GetPath() const { return file_path_; }
        void Close();
        ~ZwFile();
    ...
    public:
        FileFlt(const std::WString& current_path, const PFLT_FILTER p_filter_handle, const PFLT_INSTANCE p_instance, const PFILE_OBJECT p_file_object = nullptr, ULONG create_disposition = FILE_OPEN);
        NTSTATUS Open();
        void Close();
