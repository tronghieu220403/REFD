...
# --------------------------------------------------------------------------- #
def analyze_shannon_stats(output_file: str,
                          root_dir: str,
...
# --------------------------------------------------------------------------- #
def analyze_chi2_stats(output_file: str,
                          root_dir: str,
...
for i in range(0, len(lines), 2):
    ...
    accepted_types_in_dir[parent_dir_name].update(accepted_types[ori_type])
...
def process_file(filepath: str, file_map: dict) -> tuple[int, int, int]:
    ...
    for i in range(0, len(lines), 2):
        ...
        if len(content) > 0:
            ...
            if len(ext_list) == 0:
                no_ext += 1
            if not accepted_types_in_dir[parent_dir_name].isdisjoint(ext_list):
                matches += 1
                pass
            else:
                pass
...
if __name__ == "__main__":
    ...
    shannon_stats = analyze_shannon_stats(
        ...
        root_dir=r"E:\Graduation_Project\custom_dataset\done_real",
        threshold=7.95,
    )

    chi2_stats = analyze_chi2_stats(
        output_file="custom_chi_square_cal.txt",
        root_dir=r"E:\Graduation_Project\custom_dataset\done_real",
        threshold=311,
    )
    ...
    input_folder = r"E:\Graduation_Project\custom_dataset\code\report"
    output_file = "custom_dataset_latex_table.txt"
    ...
    with open(output_file, "w", encoding="utf-8") as out:
        ...
        for idx, fname in enumerate(sorted(os.listdir(input_folder)), 1):
            if "Sarcoma".lower() not in fname.lower():
                pass
                #continue
