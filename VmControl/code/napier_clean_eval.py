...
def format_ratio(name, value):
    ...
    percent_value = (1 - clamped_value) * 100  # giá trị từ 0–100
    ...
    if percent_value < 20:
        ...
        base_color = "cyan"
        intensity = 60 - int((percent_value - 60)) * 1
    else:
        base_color = "green"
...
# --------------------------------------------------------------------------- #
def analyze_shannon_stats(input_dir: str,
                          root_dir: str,
...
# --------------------------------------------------------------------------- #
def analyze_chi2_stats(input_dir: str,
                          root_dir: str,
...
# --------------------------------------------------------------------------- #
def process_file(filepath: str, file_map: dict) -> tuple[int, int, int, int]:
    """
    ...
    for i in range(0, len(lines), 2):
        ...
        if file_ext == "pdf" and ext_name != file_ext and ext_name != "":
            continue
        ...
        if len(content) > 0:
            ...
            no_ext += 1
    return total, matches, no_ext, ext_name
...
if __name__ == "__main__":
    ...
    shannon_stats = analyze_shannon_stats(
        input_dir=r"E:\Graduation_Project\custom_dataset\code\report_napier_clean\shannon",
        root_dir=r"E:\Graduation_Project\napier_dataset_total_clean",
        threshold=7.95,
    )

    chi2_stats = analyze_chi2_stats(
        input_dir=r"E:\Graduation_Project\custom_dataset\code\report_napier_clean\chi2",
        root_dir=r"E:\Graduation_Project\napier_dataset_total_clean",
        threshold=311,
    )
    ...
    input_folder = r"E:\Graduation_Project\custom_dataset\code\report_napier_clean\trid"
    output_file = "nap_clean_dataset_latex_table.txt"
    ...
    with open(output_file, "w", encoding="utf-8") as out:
        out.write("\\textbf{No.} & \\textbf{Ext.} & \\textbf{Total} & "
                  "\\textbf{Rejected Ext.} & \\textbf{Ratio} & "
        ...
        map_ext_by_dir = dict[str, str]()
        ext_name_info = dict[str, tuple[int, int, int]]() # {ext: (total, matches, null_ext)}
        for idx, fname in enumerate(sorted(os.listdir(input_folder)), 1):
            if "ZIP" in fname:
                pass
            fpath = os.path.join(input_folder, fname)
            total, matches, null_ext, ext_name = process_file(fpath, file_map)
            map_ext_by_dir[fname] = ext_name
            if ext_name not in ext_name_info:
                ext_name_info[ext_name] = (0, 0, 0)
            ext_name_info[ext_name] = (
                ext_name_info[ext_name][0] + total,
                ext_name_info[ext_name][1] + matches,
                ext_name_info[ext_name][2] + null_ext,
            )
        idx = 0
        for ext_name, _ in ext_name_info.items():
            idx += 1
            total, matches, null_ext = ext_name_info[ext_name]
            ...
            sh_high = sh_ratio = 0
            shannon_fam_stat = shannon_stats.get(ext_name)
            if shannon_fam_stat:
            ...
            chi2_fam_stat = chi2_stats.get(ext_name)
            if chi2_fam_stat:
            ...
            top2_keys = [k for k, v in sorted(ratios.items(), key=itemgetter(1), reverse=False)[:2]]

            ext_name = ext_name.upper()

            # ----- write LaTeX row ------------------------------------------ #
            out.write(f"{idx} & {ext_name} & "
        ...
        # ----- write average row ------------------------------------------- #
        if ext_count:
            avg_total = total_sum / ext_count
            avg_unrecog = unrecog_sum / ext_count
            avg_null_ext = null_ext_sum / ext_count
            avg_sh_high = sh_high_sum / ext_count
            avg_chi2_high = chi2_high_sum / ext_count
            ...
            out.write(f"\\multicolumn{{2}}{{l}}{{\\textbf{{Average}}}} & "
                    f"{avg_total:.1f} & {avg_unrecog:.1f} & {format_ratio('unrecog', avg_ratio_unrecog)} & "
                    f"{avg_null_ext:.1f} & {format_ratio('null', avg_ratio_null)} & "
                    f"{avg_sh_high:.1f} & {format_ratio('shannon', avg_ratio_shannon)} & "
                    f"{avg_chi2_high:.1f} & {format_ratio('chi2', avg_ratio_chi2)} \\\\\n")


            out.write("\n\n")
            out.write(f"Total families: {ext_count}\n")
